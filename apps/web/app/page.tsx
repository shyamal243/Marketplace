...
export default function HomePage() {
  ...
  function loadDemands(searchVal?: string, maxBudgetVal?: string) {
    setLoading(true);
    let url = "/demands?status=open";
    if (searchVal) url += `&search=${encodeURIComponent(searchVal)}`;
    if (maxBudgetVal) url += `&maxBudget=${encodeURIComponent(maxBudgetVal)}`;
  ...
  return (
    <div className="min-h-screen bg-indigo-deep">
      ...
      <main className="mx-auto max-w-4xl px-8 py-12 md:px-16">
        ...
        <form onSubmit={handleFilter} className="mt-4 flex gap-2">
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search demands..."
            className="flex-1 rounded-md border border-indigo-border/40 bg-indigo-surface px-3 py-2 font-body text-sm text-paper outline-none focus:border-marigold"
          />
          <input
            type="number"
            value={maxBudget}
            ...
            className="w-32 rounded-md border border-indigo-border/40 bg-indigo-surface px-3 py-2 font-body text-sm text-paper outline-none focus:border-marigold"
          />
          <button
            type="submit"
            className="rounded-md border border-indigo-border/60 px-4 py-2 font-body text-sm text-paper transition hover:border-marigold hover:text-marigold"
          >
            Filter
          </button>
        </form>
